...
interface Store {
	...
	// Remembered status filter and sort per watched-list view mode (all/tv/movie),
	// so switching between shows and movies keeps each mode's own filters & sort.
	filterModes: Record<string, string[]>;
	sortModes: Record<string, ActiveSort>;
 ...
 * filter and sort last used in that mode.
 */
